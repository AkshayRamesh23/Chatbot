...
from chat_history_db import is_u_number_in_database, fetch_email_from_database, fetch_name_from_database, insert_new_user_into_database, fetch_user_from_database, insert_conversation, fetch_conersations_from_database
...
def on_click_callback():
    ...
    with get_openai_callback() as cb:
        ...
        ######################################################################################        
        # if no u_number, then process prompt to get u_number 
        # check if u_number exists in database, if so, populate name, email and status from database
        ######################################################################################
        if st.session_state.u_number == "": 
            # get the human prompt in session state (read from text field)
            print(">>>> CHECKING EMAIL, U-NUMBER, AND NAME <<<<", datetime.now())
            
            # get the user's u_number
            
            # call llm to extract u_number, 
            response = st.session_state.llm.predict(
                f"""[INST]Extract an u_number this from this message.
            ...
            if response == "missing":
                st.session_state.history.append(
                    Message("ai", f"Pardon me, I was unable to extract your U-Number, Name and email address. Please try again!") 
                )
            else:
                st.session_state.u_number = response
                if is_u_number_in_database(st.session_state.u_number):
                    (st.session_state.u_number, st.session_state.name, st.session_state.email, st.session_state.previous_conversation) = fetch_user_from_database(st.session_state.u_number)
                    ...
                    st.session_state.history.append(                        
                        #Message("ai", f"Welcome back {st.session_state.name}!\n\n\nI see that you have interacted with me before. Also, I see that your status at USF is '{st.session_state.status}'.\n\nI am loading your conversation history '{st.session_state.conversation}'.\n\nWhat can I assist you with today?")
                        Message("ai", f"Welcome back {st.session_state.name}!\n\n\nI see that you have interacted with me before.\n\n What can I assist you with today?")
                    )
                else:
                    st.session_state.history.append(
                        Message("ai", f"Thank you. I have recorded your U-Number as {st.session_state.u_number}. Before I can answer your questions, please provide me with your name and email address.")
                    )
                    
        ###################################################        
        # if no name, then this U_NUMBER wasn't in the database
        ###################################################
            ...
            response_email = st.session_state.llm.predict(
                f"""[INST]Extract an email address from this message.
                    For any missing value, return the word missing[/INST]
            ...
            if response_name == "missing":
                ...
                st.session_state.email = response_email
                try:
                    mail.welcome_send_mail(response_email)
            ...
            #insert_conversation(u_number=st.session_state.u_number, conversation= json.dumps(messages_to_dict(st.session_state.buffer_memory.chat_memory.messages)))
...
if st.session_state.u_number == "" and st.session_state.u_number!="missing": # if this is the first time the user is chatting with the bot
    st.session_state.history.append(
        Message("ai", "Hello! I am Rocky the Bull, the chatbot for the MS BAIS program at the University of South Florida. Before I can assist you, please provide your U-Number.") 
    )
...
with prompt_placeholder:
    ...
    col1.text_input(
        "Chat",
        placeholder="Please enter your U-Number here",
...
muma-msbais@usf.edu  ''')
    ...
    if open_modal:
        ...
        insert_conversation(u_number=st.session_state.u_number, conversation=json.dumps(st.session_state.history))
 
        mail.end_send_mail(st.session_state.email,chat_list)
